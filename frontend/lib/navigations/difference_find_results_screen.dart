...
import 'package:magicmind_puzzle/constants/styles.dart';
import 'package:magicmind_puzzle/models/session_provider.dart';
import 'package:magicmind_puzzle/navigations/difference_levels_screen.dart';
import 'package:magicmind_puzzle/navigations/feedback_screen.dart';
...
class DifferenceFindResultsScreen extends StatefulWidget {
  ...
  @override
  _DifferenceFindResultsScreenState createState() =>
      _DifferenceFindResultsScreenState();
}

class _DifferenceFindResultsScreenState
    extends State<DifferenceFindResultsScreen> {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(ENVConfig.serverUrl +
            '/difference-identifications'), // Replace with your API URL
        headers: {
          'Content-Type': 'application/json',
          'Authorization':
              'Bearer YOUR_ACCESS_TOKEN', // Include token if needed
        },
        body: jsonEncode({
          'accuracy': accuracy,
          ...
          'user': user,
          'activity':
              "Difference Identification Level - " + difficulty.toString(),
      ...
      if (response.statusCode == 201) {
        ...
        print(
            "Failed to save score: \${response.statusCode} - \${response.body}");
      }
    } catch (e) {
  ...
  Future<void> _fetchIDRecords() async {
    ...
    final response = await http.get(
      Uri.parse(
          ENVConfig.serverUrl + '/difference-identifications/user/$username'),
    );
  ...
  Future<void> _generateAndShowPDF() async {
    ...
    String furtherAdvice = await _fetchNVLDAdvice(
        widget.score, widget.timeTaken, widget.difficulty);
    await _fetchIDRecords();
      ...
      pw.Page(
        build: (context) => pw.Column(
          ...
          children: [
            ...
            pw.Text(
                "- Encourage daily problem-solving activities to improve attention."),
            pw.Text(
                "- Promote teamwork and communication during similar activities."),
            pw.Text(
                "- Provide rewards for small achievements to boost motivation."),
            pw.Text(
                "- Monitor time and focus to better understand the child’s progress."),
            pw.Text(
                "- Consider consulting with a professional if NVLD traits are suspected."),
          ],
        ),
      ),
    );
  ...
  Future<String> _fetchNVLDAdvice(
      int score, int timeTaken, int difficulty) async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          "query":
              "summary of cognitive activity. Score: $score, Time Taken: $timeTaken, Difficulty: $difficulty",
          "score": score,
          "level": difficulty
        }),
      );
              ...
              child: AppBar(
                ...
                leading: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(
                        color:
                            Colors.orangeAccent.withOpacity(0.6), // Glow effect
                        blurRadius: 10,
                        spreadRadius: 2,
                      ),
                    ],
                ...
                actions: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color:
                          Color(0xff80ca84), // Background color for the circle
                      shape: BoxShape.circle, // Circular shape
                      boxShadow: [
                        BoxShadow(
