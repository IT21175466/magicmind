...
class _FeedbackScreenState extends State<FeedbackScreen> {
  ...
  Future<void> submitFeedback() async {
    ...
    int changeIdentificationDifficulty =
        prefs.getInt('difference_difficulty') ?? 1;
    final feedbackData = {
      "user": widget.user,
      ...
      if (response.statusCode == 201) {
        ...
        if (predictedDifficulty != null) {
          ...
          if (changeIdentificationDifficulty < predictedDifficulty) {
            ...
            message =
                'Keep it going! Your level is already matching the predicted value of $predictedDifficulty.';
          }
          showDialog(
            context: context,
              ...
              content: Column(
                ...
                children: [
                  Text(
                    'LEVEL',
                    style: TextStyle(
                        fontSize: 15, color: Colors.white), // Optional styling
                  ),
                  Text(
                    '$predictedDifficulty',
                    style: TextStyle(
                        fontSize: 56, color: Colors.white), // Optional styling
                  ),
                ...
                TextButton(
                  onPressed: () {
                    Navigator.pushReplacement(
                      ...
                      MaterialPageRoute(
                          builder: (context) =>
                              DifferenceRecognitionLevelsScreen()),
                    );
                  },
                  child: Text('Cancel'),
                ),
