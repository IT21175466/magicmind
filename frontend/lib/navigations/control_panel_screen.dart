...
class _ControlPanelScreenState extends State<ControlPanelScreen> {
  ...
  // Load and initialize difficulty values
  Future<void> _loadDifficulty() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    // Initialize all difficulties to 1 if not set
    setState(() {
      _differenceDifficulty =
          (prefs.getInt('difference_difficulty') ?? 1).clamp(1, 5);
      _puzzleDifficulty = (prefs.getInt('puzzle_difficulty') ?? 1).clamp(1, 5);
      _prepositionDifficulty =
          (prefs.getInt('preposition_difficulty') ?? 1).clamp(1, 5);
      _vocabularyDifficulty =
          (prefs.getInt('vocabulary_difficulty') ?? 1).clamp(1, 5);
    });

    // Save initial values to ensure they are set to 1
